...
function loadComments(medicineId) {
    spinner()
    fetch(`/api/medicine/${medicineId}/comments`).then(res => res.json()).then(data => {
        spinner("none")
        let h = ""
        data.forEach(c => {
            h += `
...
function addComment(medicineId) {
    spinner()
     fetch(`/api/medicine/${medicineId}/comments`, {
        method: "post",
        body: JSON.stringify({
